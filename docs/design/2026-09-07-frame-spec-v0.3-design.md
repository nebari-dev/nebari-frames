...
**8 of 9 journey items verified. Journey 4 is partially verified and is not claimed as passing.**

The validator, its fixtures and both conformance profiles are built, reviewed and pushed
as `spec/v0.3-validator` in `openteams-ai/frame-spec`, 28 signed commits, 171 tests, with
continuous integration green. Every row's evidence above was captured fresh at that branch.

Journey 4's divergences are defects in this specification, not in the reader that found
them, and both are the specification author's to resolve.
...
one-value-per-section reading. A sentence in section 6.2.2 settling cardinality the way
section 4.2.2 settles it for `guidance` would close both.
...
the section that defines extraction.
...
committed fixture has two. Whichever way that resolves, `expected-model.json` is
regenerated.
...
- **Tracks.** A Track records "the Frames applied," and pre-flight Guards check "whether the right Frames were applied." Both need to name a Frame unambiguously. `identifier`, `version`, and the Representation's `checksum` are what a Track records; nothing else in the model is required for that purpose.
