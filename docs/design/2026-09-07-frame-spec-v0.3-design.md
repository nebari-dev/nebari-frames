...
### Journey drift, declared 2026-09-10

Journey 4's criterion asked for a reader. The test run on 2026-09-10 asked for a reader
and a writer, from two independent builds instead of one, and judged them against each
other as well as against the reference implementation. The criterion is strengthened
rather than met as written, which is declared here rather than left to look as though it
always said this.

One criterion of the test design was dropped before it ran, and the reason is worth
recording: an early draft judged each build's Markdown output against the released v0.2
validator. That is not a conformance criterion. Compatibility is backward only, so a v0.3
reader must read a v0.2 document, and Section 6.2.1 only recommends that a writer emit all
four v0.2 keys. Passing the v0.2 validator is therefore an observation about a SHOULD, not
a gate. The backward-compatibility proof lives on the reader side, where accepting all 18
v0.2 examples is exactly that test.

### Verification status, 2026-09-09

**9 of 9 journey items verified**, with journey 4 re-verified on 2026-09-10 against a strengthened criterion covering a writer as well as a reader, by two independent builds rather than one. Journey 4 passed on a second, independent build after the specification was amended to settle what the first build's divergences exposed. Its original partial result and both divergences are kept above and below rather than overwritten.
