...
// Presentational, data-agnostic drawing of an inheritance graph. `focus` (an
// "org/name" id) highlights that frame's lineage and dims everything else.
export function FrameHierarchyView({ graph, focus = "" }: { graph: HierarchyGraph; focus?: string }) {
  const lineage = useMemo(
    () => (focus ? connectedTo(graph, focus) : new Set<string>()),
    [graph, focus],
  );
  const { placed, width, height } = useMemo(() => place(graph), [graph]);

  // Only apply focus styling when the focused frame is actually in the graph; a
  // stale focus id falls back to the normal, undimmed view.
    ...
    <div className="overflow-x-auto rounded-lg border bg-muted/20 p-6">
      <div className="relative mx-auto" style={{ width: `${width}px`, height: `${height}px` }}>
        ...
        {[...placed.values()].map((node) => (
          <NodeCard key={node.id} node={node} focused={node.id === focus} dimmed={dimmed(node.id)} />
        ))}
      </div>
    </div>
  );
